...
The `r which(StepsPerInterval==max(StepsPerInterval))`rd 5-minute interval, on average across all the days in the dataset, 
contains the maximum number of steps (`r max(StepsPerInterval)`).
...
The number of rows with missing values is `r length(ok)-sum(ok)`.
